...
{
	//test memory
	int* ptr = (int*)malloc(500 * 1024 *1024);
	if (!ptr){
	...
	if (g_bus_num != 0 && g_device_addr != 0) {
		...
		if (jtag_libusb_open(ch347_vids, ch347_pids,
			...
			if (!(dev = libusb_get_device(ch347_handle))) {
				return false;
			}
...
static bool CH347WriteData(uint64_t iIndex, uint8_t *data, uint64_t *length)
{
	...
	if (!ret)
		...
		LOG_DEBUG_IO("ch347, outlen = %ld outed_lenght = %d, result = %d", *length, tmp, ret);
		int result = libusb_clear_halt(ch347_handle, CH347_EPOUT);
		if(result == 0){
...
#elif defined(__linux__)
	libusb_set_auto_detach_kernel_driver(ch347_handle,4);
	
	if (LIBUSB_SUCCESS != libusb_claim_interface(ch347_handle, 4)) {
		// printf("libusb_claim_interface 4 failed!!!\n");
	}
#endif
	if (!swd_mode) {
